...
{
    [Generator]
    public class NotifyPropertyChanged : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            // Получаем все поля с атрибутом [AutoNotify]
            ...
            // Собираем дерево
            var compilationAndFields = context.CompilationProvider.Combine(fieldsWithAttribute.Collect());
        ...
        private static (IFieldSymbol fieldSymbol, string propertyName, string[] props)? GetFieldWithAttribute(GeneratorSyntaxContext context)
        {
            ...
            foreach (var variable in field.Declaration.Variables)
            {
                ...
                if (symbol is IFieldSymbol fieldSymbol &&
                    fieldSymbol.GetAttributes().Any(ad => ad.AttributeClass?.ToDisplayString() == "RetailCorrector.Attributes.NotifyUpdatedAttribute"))
                {
                    var attr = fieldSymbol.GetAttributes().First(ad => ad.AttributeClass?.ToDisplayString() == "RetailCorrector.Attributes.NotifyUpdatedAttribute");
        ...
        private static void Execute(Compilation compilation, ImmutableArray<(IFieldSymbol fieldSymbol, string propertyName, string[] props)?> fields, SourceProductionContext context)
        {
            foreach (var group in fields.Where(f => !(f is null)).GroupBy(f => f.Value.fieldSymbol.ContainingType))
            {
                var classSymbol = group.Key;
                var source = GenerateClass(group.Select(q=>q.Value).ToList());
                context.AddSource($"{classSymbol.Name}_NotifyUpdated.g.cs", SourceText.From(source, Encoding.UTF8));
            }
        }
        ...
        private static string GenerateClass(List<(IFieldSymbol fieldSymbol, string propertyName, string[] props)> fields)
        {
            ...
            var sb = new StringBuilder($@"using System.ComponentModel;
